...
breedte = 1700
hoogte = 300
resolutie = str(breedte) + "x" + str(hoogte)
# resolutie = "1700x300"
...
class Game(Frame):
	def __init__(self, master=None, **kwargs):
		...
		self.canvas = Canvas(master, width=breedte, height=hoogte, background="black")
	...
	def RandomXY(self, p_lane):
		LaneWidth = breedte // self.lanes
		x = random.randint(LaneWidth * p_lane, LaneWidth * (p_lane + 1) - self.breedte_image)
		y = random.randint(0, hoogte - self.breedte_image)
		# print(str(hoogte) + " " + str(self.breedte_image))
		return x, y
	...
	def No_shoot(self, i):
		if random.randint(0, 100) < 50:
			...
			i = 0
			while i < len(self.coordinates_list):
				if sqrt(((self.coordinates_list[i][0] - x) **2) + ((self.coordinates_list[i][1] - y) **2)) < self.breedte_image:
					x = random.randint(LaneWidth * (i) + 10, (LaneWidth * (i + 1) - self.breedte_image) - 10)
					y = random.randint(0, hoogte - self.breedte_image)
					i = 0
					continue
